...
class StoneCrusher extends Model
{
    use HasFactory;
    protected $table = 'stonecrusher';
    ...
    protected static function boot()
    {
        ...
        static::creating(function ($stonecrusher) {
            // Generate kode otomatis saat membuat data baru
            $stonecrusher->kode = self::generateKodeStoneCrusher();
        });
    }

    public static function generateKodeStoneCrusher()
    {
        // Ambil kode stonecrusher terakhir
        $stonecrusherID = self::orderBy('id', 'desc')->first();
        ...
        if (!$stonecrusherID) {
            ...
            $lastCode = $stonecrusherID->kode;
            $lastNumber = (int) substr($lastCode, 7); // DRV-001 -> ambil 001
            $number = $lastNumber + 1;
        }

        // Format angka dengan leading zeros (3 digit)
        $formattedNumber = str_pad($number, 7, '0', STR_PAD_LEFT);

        return 'SCID-' . $formattedNumber;
    }
